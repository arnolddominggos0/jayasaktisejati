...
class ImportTamJune2026Units extends Command
{
    ...
    protected $description = 'Import TAM June 2026 Units dari JSON';
    ...
    {
        $path = storage_path('imports/tam/june2026/units.json');
        ...
        if (empty($rows)) {
            $this->error('Tidak ada data unit yang ditemukan.');
            return self::FAILURE;
        }
        ...
        $this->newLine();
        $this->line('<fg=cyan>============================================</>');
        $this->line('<fg=cyan>  VALIDASI IMPORT UNIT TAM JUNI 2026   </>');
        $this->line('<fg=cyan>============================================</>');
        $this->table(
            ['Metrik', 'Jumlah'],
            [
