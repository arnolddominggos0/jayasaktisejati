...
class ShipmentResource extends Resource
{
    ...
    protected static function resolveTimelineMask(Shipment $m): array
    {
        ...
        $fallback = $profiles[$fallbackKey] ?? ['show_planning' => true, 'show_terminal_detail' => true, 'show_legacy' => false];
        ...
        $matches = function (array $when) use ($mode, $originBr, $destBr): bool {
            if (isset($when['mode']) && strtolower($when['mode']) !== $mode) return false;
            if (isset($when['branch_id_in']) && !in_array($originBr, (array)$when['branch_id_in'], true)) return false;
            if (isset($when['not_branch_id_in']) && in_array($originBr, (array)$when['not_branch_id_in'], true)) return false;
            if (isset($when['dest_branch_id_in']) && !in_array($destBr, (array)$when['dest_branch_id_in'], true)) return false;
            return true;
        };
                ...
                \Filament\Tables\Actions\Action::make('createAssignment')
                    ...
                    ->visible(
                        fn(Shipment $record) => ($record->mode === ShipmentMode::Land)
                            && in_array(($record->status?->value ?? (string)$record->status), array_map(fn($s) => $s->value, ShipmentStatus::active()), true)
                    ),
