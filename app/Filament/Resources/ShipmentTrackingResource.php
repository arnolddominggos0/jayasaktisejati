        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('route_summary')->label('Rute')->html()
                    ->getStateUsing(
                        fn (Shipment $r) => "<div class='font-medium'>".($r->originCity->name ?? '-').' &rarr; '.($r->destinationCity->name ?? '-').'</div>'
                    )->toggleable(),
                ...
                Filter::make('manado_kpi_target')->label('Target KPI Manado')
                    ->query(function (Builder $q) {
                        ...
                        $q->where(function ($w) use ($branchIds, $cityIds, $depotIds) {
                            ...
                            if (! empty($cityIds)) {
                                $w->orWhereIn('destination_city_id', $cityIds);
                            }
                            if (! empty($depotIds)) {
                                $w->orWhereIn('assigned_depot_id', $depotIds);
                            }
                        });
                    })
                    ->toggle(),
