...
class VoyageScheduleHistoryFeb2026Seeder extends Seeder
{
    ...
    private array $voyages = [
        [
            ...
            'draft' => [
                'etd' => '2026-02-08',
                'eta' => null,            // Draft hanya ETD, ETA belum ditetapkan
            ],
    ...
    public function run(): void
    {
        $this->line('');
        $this->line('╔══════════════════════════════════════════════════════════╗');
        $this->line('║   VESSEL PLAN SCHEDULE HISTORY BACKFILL — FEB 2026      ║');
        $this->line('╚══════════════════════════════════════════════════════════╝');
        $this->line('');
        ...
        $this->line(sprintf(
            '%-6s │ %-14s │ %-14s │ %-14s │ %s',
            'Voyage', 'Draft', 'Final', 'Actual', 'Status'
        ));
        ...
        foreach ($auditRows as $audit) {
            ...
            try {
                DB::transaction(function () use (
                    &$audit, $row, $vno, $action,
                    &$stats, $shippingLineId, $polId, $podId
                ) {
                    $voyage = $audit['voyage'];
                ...
                $this->line(sprintf(
                    '%-6s │ %-14s │ %-14s │ %-14s │ %s',
                    $vno, '✓ Draft', '✓ Final', '✓ Actual', $statusNote
                ));

            } catch (\Throwable $e) {
                $this->error(sprintf(
                    '%-6s │ %-14s │ GAGAL: %s',
                    $vno, 'ERROR', $e->getMessage()
                ));
            }
        }
