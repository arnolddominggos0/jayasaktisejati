...
--}}
<x-filament::section heading="Ringkasan Perencanaan">

    <div class="grid grid-cols-3 gap-x-8">

        <div>
            <div class="text-sm text-gray-500">Jadwal</div>
            <div class="mt-1 text-2xl font-semibold text-gray-800">{{ $scheduleCount }}</div>
        </div>

        <div>
            <div class="text-sm text-gray-500">Rencana Muatan</div>
            <div class="mt-1 text-2xl font-semibold text-gray-800">{{ $cargoTotal }} <span class="text-base font-normal text-gray-500">unit</span></div>
        </div>

        <div>
            <div class="text-sm text-gray-500">ETD Gap</div>
            <div class="mt-1 text-2xl font-semibold {{ $gapOk ? 'text-gray-800' : ($maxGap <= 10 ? 'text-amber-600' : 'text-red-600') }}">
                {{ $maxGap }} <span class="text-base font-normal text-gray-500">hari</span>
            </div>
            <div class="mt-0.5 text-sm text-gray-500">Target &le; {{ $idealGap }} hari</div>
        </div>
