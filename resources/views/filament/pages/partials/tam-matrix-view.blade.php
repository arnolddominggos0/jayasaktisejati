                ...
                @foreach ($sorted as $index => $v)
                    ...
                    <tr class="transition border-b border-gray-100/20 {{ $rowBorder }} {{ $rowBg }} {{ $zebra }} hover:bg-gray-50/30">
                        ...
                        <td class="px-2 py-1.5 sticky left-0 z-10 border-r border-gray-200/20 {{ $rowBg ?: ($zebra ?: 'bg-white') }} hover:bg-gray-50/30">
                            <div class="font-semibold text-gray-900 text-[11px] truncate leading-tight">{{ $v->vessel?->name }}</div>
                        ...
                        {{-- Actions: ultra-subtle ghost, monochrome --}}
                        <td class="px-2 py-1.5 text-center">
                            <div class="flex items-center justify-center gap-0.5">
                                ...
                                @endif
                                @if ((count($criticalIssues) || count($secondaryIssues)) && !in_array($v->id, $acknowledged))
                                    <button wire:click="acknowledgeVoyage({{ $v->id }})"
                                        class="inline-flex items-center justify-center w-5 h-5 rounded text-gray-400 hover:text-green-600 hover:bg-green-50/60 transition"
                                        title="Acknowledge">
