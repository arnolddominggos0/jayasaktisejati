...
{{-- The Matrix is "Papan Operasional" (Fleet Board) — the operational
     center, not a deliberately-reached Reference table. Visual weight is
     lowered relative to the Fleet Status / Operational Brief above it, so
     this heading is not the strongest on the page. --}}
...
@else
    <div class="bg-white border border-gray-200/40 rounded-lg overflow-hidden divide-y divide-gray-100/60">
        @foreach ($sorted as $index => $v)
            @php
                // The Fleet Board derives its own condition independently from
                // already-loaded facts, never via TaskClassifier/getBrief().
                ...
                // Current Position — Berlayar/Selesai/Terjadwal plus a DELAYED
                // case, so "where is this voyage" always answers something.
                // Only DELAYED stays red (severity); the healthy states are
                // quiet graphite differentiated by weight, never by color.
                $position = match (true) {
                ...
                // Temporal register (anticipation layer): pure Blade derivation
                // from already-loaded etd/eta vs now(), never a new query.
                // Shown only for voyages with no active severity issue —
                // severity always outranks temporal weight, so a flagged voyage
                // never also carries an anticipation marker. Non-alarm and
                // subordinate: amber-for-today (imminent), muted gray-for-tomorrow
                // (soon). Distant future stays silent (null).
                $temporal = null;
                if (! $hasIssues) {
                ...
                // Monochrome progress rail — 4-glyph grammar:
                // ▰ completed (gray-600) · ▱ current/you-are-here (gray-900)
                // · future (gray-300) · ◌ missing-expected/late (red-600).
                // Absence-as-signal: a stage that should have happened and
                // didn't becomes the one loud mark. Derived from atb/atd/ata/
                // closing timestamps, milestone actual_date/is_overdue, OTD/OTA late.
                $stageDefs = [
            ...
            {{-- One voyage block. --}}
            {{-- Verification settle: after a successful RecordingModal save,
                 the just-updated voyage briefly receives elevated weight (a
                 quiet inset ring + raised surface), then settles back to its
                 resting state after ~1.6s via the `transition` class. No
                 keyframe animation, no color — weight only, so the operator
                 sees the state change land. --}}
            <div wire:key="voyage-row-{{ $v->id }}"
                ...
                {{-- Breathing room (py-3.5) and clear group separation (gap-x-8)
                     — spacing only, no new border, no new color. --}}
                <div class="px-4 py-3.5 flex flex-wrap items-start gap-x-8 gap-y-2.5">
                    ...
                    <div class="w-28 shrink-0">
                        ...
                        @if ($d1 || $h1)
                            {{-- Readiness completion is monochrome (done = quiet
                                 gray-600, not green). Only late stays red (severity). --}}
                        ...
                        @if ($temporal)
                            {{-- Anticipation marker — non-alarm, subordinate,
                    ...
                    <div class="w-48 shrink-0">
                        ...
                        @else
                            {{-- Healthy condition is quiet, not green:
                                 "On Track" recedes at gray-400. --}}
                            <span class="text-gray-400 text-[10px] font-medium">On Track</span>
                        @endif
                    </div>

                    {{-- OPERATIONAL RESPONSIBILITY + Muatan + Quick Action —
                         grouped under one label instead of a bare "Aksi" column.
                         Severity accent border on the left when there are issues. --}}
                    <div class="flex-1 min-w-[220px] {{ $hasIssues ? 'border-l pl-3 ' . str_replace('border-l-', 'border-l-', $rowBorder) : '' }}">
                        ...
                        <div class="flex items-center flex-wrap gap-2">
                            ...
                            {{-- Muatan: cargo plan/actual + input button --}}
                            <div class="text-[10px]">
                                @if ($v->cargo_actual !== null)
                                    ...
                                    @if ($v->cargo_plan !== null)
                                        ...
                                        {{-- Cargo variance is informational, not a
                                             severity alarm: monochrome, never green/red.
                                             A shortfall reads as heavier gray (notable),
                                             a surplus/exact as quiet gray. --}}
                            ...
                            <div class="flex items-center gap-0.5">
                                ...
                                {{-- A recorded actual is confirmation, not celebration:
                                     filled quiet gray, never green. Unrecorded stays a
                                     neutral affordance. --}}
                    ...
                    {{-- OPERATIONAL PROGRESS — monochrome lifecycle rail,
                         5-stage flow: ATB → ATD → Sailing[D+2→D+4→D+6] → ATA
                         → Closing. 4-glyph grammar: ▰ completed (gray-600) ·
                         ▱ current (gray-900) · · future (gray-300) · ◌ missing-
                         expected/late (red-600). A late departure/arrival reads
                         as the red ◌ (or a red ▰ if recorded-but-late) at its
                         own stage. --}}
    ...
    {{-- Persistent legend for the progress glyph grammar. Monochrome, one
         reserve hue. --}}
